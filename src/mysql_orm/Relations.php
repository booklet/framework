...
class Relations
{
    ...
    public function sqlQuery()
    {
        foreach ($this->obj_class_name::relations() as $relation_fn_name => $relation_params) {
            if ($relation_fn_name == $this->fn_name) {
                if ($relation_params['relation'] == 'has_many') {
                    $underscore_class_name = Util::camelCaseStringToUnderscore($this->obj_class_name);
    ...
    {
        return Util::camelCaseStringToUnderscore($class_name) . '_id';
    }

    private function getHABTMTableName($relation_params)
    {
        $habtm_model_name_arr = [];
        $habtm_model_name_arr[] = Util::camelCaseStringToUnderscore($this->obj->pluralizeClassName());
        $habtm_model_name_arr[] = Util::camelCaseStringToUnderscore((new $relation_params['class'])->pluralizeClassName());
